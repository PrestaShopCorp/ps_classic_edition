...
if [ $# -le 1 ]; then
  ...
  echo "tests/phpstan/phpstan.sh [PrestaShop_version] [PHP_version]"
  exit 1
fi
...
docker exec -t temp-ps rm -rf /var/www/html/modules/ps_classic_edition

echo "Run PHPStan using phpstan-${PS_VERSION}.neon file"
docker run --rm --volumes-from temp-ps \
       -v $PWD:/var/www/html/modules/ps_classic_edition \
       -e _PS_ROOT_DIR_=/var/www/html \
       -e DISABLE_MAKE=1 \
       --workdir=/var/www/html/modules/ps_classic_edition ghcr.io/phpstan/phpstan:1.10.45-php${PHP_VERSION} \
       analyse \
       --error-format=github \
       --configuration=/var/www/html/modules/ps_classic_edition/tests/phpstan/phpstan-${PS_VERSION}.neon
